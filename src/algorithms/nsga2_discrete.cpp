...
nsga2_discrete::nsga2_discrete(unsigned gen, double cr, double eta_c, double m, unsigned seed)
    : m_gen(gen), m_cr(cr), m_eta_c(eta_c), m_m(m), m_e(seed), m_seed(seed), m_verbosity(0u)
{
    if (cr >= 1. || cr < 0.) {
 ...
 * 4.
 */
population nsga2_discrete::evolve(population pop) const
{
    // We store some useful variables
    const auto &prob = pop.get_problem(); // This is a const reference, so using set_seed for example will not be
                                          // allowed
    const auto discrete_variables = pop.get_problem().get_discrete_variables();
    ...
    for (decltype(m_gen) gen = 1u; gen <= m_gen; gen++) {
        ...
        if (m_bfe) {
            ...
            for (decltype(NP) i = 0u; i < NP; i += 4) {
                ...
                children = detail::sbx_crossover_discrete_impl(pop.get_x()[parent1_idx], pop.get_x()[parent2_idx], m_cr, m_e);
                detail::polynomial_mutation_discrete_impl(children.first, discrete_variables, m_m, m_e);
                detail::polynomial_mutation_discrete_impl(children.second, discrete_variables, m_m, m_e);
 ...
 */
void nsga2_discrete::set_seed(unsigned seed)
{
    m_e.seed(seed);
    m_seed = seed;
}
 ...
 */
void nsga2_discrete::set_bfe(const bfe &b)
{
    m_bfe = b;
}
 ...
 */
std::string nsga2_discrete::get_extra_info() const
{
...
void nsga2_discrete::serialize(Archive &ar, unsigned)
{
...
PAGMO_S11N_ALGORITHM_IMPLEMENT(pagmo::nsga2_discrete)
